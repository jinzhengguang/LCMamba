...
if __name__ == "__main__":
    ...
    # 输入示例：batch_size=4, channels=256, height=180, width=180
    # torch.Size([4, 256, 180, 180]) 
    x = torch.randn(4, 3, 224, 224).to(device)
    se_block = VSSM(in_chans=3,
                num_classes=1,
                depths=[2, 2, 2, 2],
    ...
    y = se_block(x)
    print(y.shape)  # 输出应为 torch.Size([4, 256, 180, 180])

    x = torch.randn(4, 256, 180, 180).to(device)
    x = x.permute(0, 2, 3, 1)
    blocks = VSSBlock(hidden_dim=256).to(device)
    y = blocks(x)
    y = y.permute(0, 3, 1, 2)
    print('2024-11-06 Jinzheng Guang')

# 2024-11-06 Jinzheng Guang 
